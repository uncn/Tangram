...
import com.sunzn.tangram.library.helper.SizeHelper;
...
public class LoadingView extends View {
    ...
    public LoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSize = array.getDimensionPixelSize(R.styleable.LoadingView_view_size, SizeHelper.dp2px(context, 32));
    ...
    private void drawLoading(Canvas canvas, int rotateDegrees) {
        ...
        canvas.rotate(rotateDegrees, mSize / 2.0f, mSize / 2.0f);
        canvas.translate(mSize / 2.0f, mSize / 2.0f);

        for (int i = 0; i < LINE_COUNT; i++) {
            canvas.rotate(DEGREE_PER_LINE);
            mPaint.setAlpha((int) (255f * (i + 1) / LINE_COUNT));
            canvas.translate(0, -mSize / 2.0f + width / 2.0f);
